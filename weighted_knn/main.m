...
prompt = 'Please select one of the following methods for Weighted kNN algorithm.\n1:Cross Validation with folds=10\n2:Percentance Split 66\n3:Leave one out\n';
method = input(prompt);
switch method
    case 1
        disp('--------------------------------------------------------------------');
        ...
        Confusion_Matrix=zeros(2);                                          %Initialization of matrix Confusion_Matrix
            for n=1:10                                                      %Runs for N folds (N=10)
                ...
                [Confusion_Matrix]=myWeightedKnn(TrainData,TestData,LabelsTrainData,LabelsTestData,Labels,k,Confusion_Matrix);  %Call myWeightedKnn function
            end
